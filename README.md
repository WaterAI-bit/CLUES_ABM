...
### Core Capabilities
* **High-Resolution Simulation**: Unfolds spatial-temporal changes dynamically, allowing high-precision simulations (such as day-by-day precision) for short and medium-term periods.
* **Socio-Economic Coupling**: Seamlessly bridges global environmental shifts with intricate socio-economic dynamics, capable of executing multi-scale simulations across **firm-, city-, national-, and global-level** granularities.
...
<img src="https://github.com/WaterAI-bit/CLUES_ABM/raw/main/docs/clues_abm_flow.png" alt="CLUES-ABM model schematic diagram" width="60%"/>
...
## 2.2 Applications & Scenario Templates
...
-   **Simulation Cycle** (e.g., 365 days a year)
    ...
    3.  Execute core agent actions:
        -   🏭 **Production Agents:**   $$\text{Production} \longrightarrow \text{Product Preparation} \longrightarrow \text{Order Placement} \longrightarrow \text{Adaptive Substitution} \longrightarrow \text{Memory Update}$$
        -   🛒 **Consumption Agents:** $$\text{Final Consumption} \longrightarrow \text{Order Placement} \longrightarrow \text{Inventory Depletion} \longrightarrow \text{Memory Update}$$
        -   🚚 **Transportation Agents:** $$\text{Cargo Loading} \longrightarrow \text{Route Latency Stepping} \longrightarrow \text{Network Friction Buffering} \longrightarrow \text{Destination Delivery}$$
...
1.  Load the data (e.g. `MRIOExample.npz` or `MRIOExample.m`)
...
-   A daily Value-Added line chart window will automatically
    pop up to visualize the economic impact.
-   Optionally, save the results to
    `TestResults_ReductionInProductionCapacityExample.npz` or `TestResults_ReductionInProductionCapacityExample.mat` for
    post-processing.
...
-   **`MRIOExample.npz`:** Represents the multi-regional input-output
...
-   **Capacity Shocks (`model.AgentsP_Theta`):** Modified directly within the example execution scripts. By setting targeted degradation ratios (bounded within $[0, 1]$) in the capacity vector over specified day intervals, users can endogenously simulate sudden drops in manufacturing, infrastructure asset downtime, or supply capacity.
-   **Resource Constraints (`model.ResourceConstraints`):** Controls the rigid spatial-temporal environmental or material boundaries of the system. By scaling this resource quota vector alongside the micro-level intensity coefficients (`model.AgentsP_ResourceIntensity`), users can programmatically simulate severe localized eco-ceilings (e.g., water scarcity, energy double-control caps) that force factory-level adaptive input substitutions.
-   **Transportation blockage (`model.AgentsT_*`):** Controls the cargo flow propulsion velocity and unloading mechanisms within transit lanes. The structural routing alignments are managed via the network topology layers `model.AgentsT_P2P` and `model.AgentsT_P2C`. By calling `model.transport_obstruct_mrio()`, users can clamp transit latencies to simulate sudden logistical chokepoint blockages (e.g., canal obstructions) that drain downstream buffers.
...
$`\langle T \rangle ^{\rightarrow \langle P \rangle}(r_1,s_1,r_2,s_2)`$, shipping the relevant intermediate product,
